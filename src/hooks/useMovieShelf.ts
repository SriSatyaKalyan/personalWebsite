...
async function loadGroup(
	...
	try {
		localStorage.setItem(
			cacheKey,
			JSON.stringify({ data: cards, ts: Date.now(), hash }),
		);
	} catch {
		/* storage full */
	}
...
export function useMovieShelf() {
	...
	useEffect(() => {
		const username = import.meta.env.VITE_LETTERBOXD_USERNAME as
			| string
			| undefined;

		async function load() {
			// Currently watching — TMDB
			const watchingCards = await loadGroup(
				currentlyWatching,
				CACHE_KEY_WATCHING,
			);

			// Manually tracked watched — TMDB
			const manualWatched = await loadGroup(
				recentlyWatched,
				CACHE_KEY_WATCHED,
			);
			...
			if (username) {
				...
				if (!lbdCards.length) {
					...
					try {
						localStorage.setItem(
							CACHE_KEY_LBD,
							JSON.stringify({ data: lbdCards, ts: Date.now() }),
						);
					} catch {
						/* storage full */
					}
			...
			const manualTitles = new Set(
				manualWatched.map((c) => c.title.toLowerCase()),
			);
			const merged = [
				...manualWatched,
				...lbdCards.filter(
					(c) => !manualTitles.has(c.title.toLowerCase()),
				),
			];
